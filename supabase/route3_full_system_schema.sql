-- Route3 / IT Shop Manager full database schema for Supabase
...
-- -----------------------------------------------------------------------------
-- Core identity and lookup tables
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- Seed lookup data and demo accounts
-- -----------------------------------------------------------------------------
insert into public.profiles (email, name, role)
values
  ('admin@itshop.com', 'John Admin', 'admin'),
  ('sales@itshop.com', 'Sarah Sales', 'sales'),
  ('inventory@itshop.com', 'Mike Inventory', 'inventory'),
  ('accountant@itshop.com', 'Lisa Accounts', 'accountant'),
  ('tech1@itshop.com', 'Alex Technician', 'technician'),
  ('tech2@itshop.com', 'Maya Technician', 'technician'),
  ('tech3@itshop.com', 'Noah Technician', 'technician')
on conflict (email) do update set
  name = excluded.name,
  role = excluded.role,
  is_active = true,
...
values ('Apple'), ('Samsung'), ('Google'), ('Xiaomi'), ('OnePlus')
on conflict (name) do nothing;

insert into public.device_colors (name)
values ('Black'), ('White'), ('Blue'), ('Silver'), ('Graphite'), ('Gold')
on conflict (name) do nothing;
...
-- -----------------------------------------------------------------------------
-- Sanity seed rows for the lab board
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- After running the migration, run the verification script in route3_integrity_check.sql.
